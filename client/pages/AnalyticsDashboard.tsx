...
import { getSalesRevenueLabel, getSalesSummaryTitle, hasLiterSales } from '@/lib/dashboard-summary';
...
const Dashboard: React.FC = () => {
  ...
  const formatSalesUnitBreakdown = (items?: Array<{ unit: string; quantity: number }>) => {
    if (!items || items.length === 0) return '0';
    return items.map((item) => formatQuantityWithUnit(item.quantity, item.unit)).join(', ');
  };

  const formatMargin = (profit: number, revenue: number) => {
    if (!revenue) return '0.0';
    return ((profit / revenue) * 100).toFixed(1);
  };

  const formatSalesQuantity = (salesData: any) => {
    const { totalLiters = 0, totalKg = 0, totalUnits = 0 } = salesData || {};
    const parts = [];
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            ...
            <p className="text-sm sm:text-base text-muted-foreground mt-1">
              Sut mahsulotlari biznesingiz uchun batafsil tahlil va ko'rsatkichlar
            </p>
          </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                  <Card>
                    ...
                    <CardContent>
                      ...
                      <p className="text-xs text-muted-foreground">
                        O'rtacha: {formatCurrency(dashboardData.summary.averagePurchasePricePerLiter)} / litr
                      </p>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                      <CardTitle className="text-xs sm:text-sm font-medium">{getSalesSummaryTitle(dashboardData.summary.salesQuantityByUnit)}</CardTitle>
                      <TrendingUp className="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                      <div className="text-base sm:text-xl font-bold">{formatSalesUnitBreakdown(dashboardData.summary.salesQuantityByUnit)}</div>
                      <p className="text-xs text-muted-foreground">
                        {formatCurrency(dashboardData.summary.totalSalesRevenue)} {getSalesRevenueLabel(dashboardData.summary.salesQuantityByUnit)}
                      </p>
                      {hasLiterSales(dashboardData.summary.salesQuantityByUnit) && (
                        <p className="text-xs text-muted-foreground">
                          Litr o'rtacha: {formatCurrency(dashboardData.summary.averageSalesPricePerLiter)} / litr
                        </p>
                      )}
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {formatMargin(dashboardData.summary.grossProfit, dashboardData.summary.totalSalesRevenue)}% marja
                      </p>
                    </CardContent>
                  </Card>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription className="text-xs sm:text-sm">Kunlik sotuv miqdori</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <ResponsiveContainer width="100%" height={250} className="sm:h-[300px]">
                        <LineChart data={dashboardData.salesOverTime}>
                          ...
                          <Tooltip
                            ...
                            formatter={(value: any, name: string, props: any) => {
                              if (name === 'Sotish' || name === 'totalQuantity') {
                                return [formatSalesQuantity(props?.payload), 'Hajm'];
                              }
                              return [value, name];
                            }}
                          />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription className="text-xs sm:text-sm">Jami sotuv miqdori va daromad</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-3 sm:space-y-4">
                        {dashboardData.customerBreakdown.map((customer, index) => (
                          <div key={customer.customerId} className="flex items-center justify-between">
                            ...
                            <div className="text-right">
                              <div className="text-xs sm:text-sm font-medium">{formatSalesUnitBreakdown(customer.salesQuantityByUnit)}</div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <Card>
                    ...
                    <CardContent>
                      ...
                      <p className="text-xs text-muted-foreground">
                        O'rtacha: {formatCurrency(allTimeData.summary.averagePurchasePricePerLiter)} / litr
                      </p>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                      <CardTitle className="text-sm font-medium">{getSalesSummaryTitle(allTimeData.summary.salesQuantityByUnit)}</CardTitle>
                      <TrendingUp className="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                      <div className="text-xl font-bold">{formatSalesUnitBreakdown(allTimeData.summary.salesQuantityByUnit)}</div>
                      <p className="text-xs text-muted-foreground">
                        {formatCurrency(allTimeData.summary.totalSalesRevenue)} {getSalesRevenueLabel(allTimeData.summary.salesQuantityByUnit)}
                      </p>
                      {hasLiterSales(allTimeData.summary.salesQuantityByUnit) && (
                        <p className="text-xs text-muted-foreground">
                          Litr o'rtacha: {formatCurrency(allTimeData.summary.averageSalesPricePerLiter)} / litr
                        </p>
                      )}
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {formatMargin(allTimeData.summary.grossProfit, allTimeData.summary.totalSalesRevenue)}% marja
                      </p>
                    </CardContent>
                  </Card>
                        ...
                        {allTimeData.customerBreakdown.map((customer, index) => (
                          <div key={customer.customerId} className="space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-2 text-xs">
                              <div>
                                <span className="text-muted-foreground">Hajm:</span>
                                <div className="font-medium">{formatSalesUnitBreakdown(customer.salesQuantityByUnit)}</div>
                              </div>
                              <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <Card>
                  ...
                  <CardContent>
                    ...
                    <p className="text-xs text-muted-foreground">
                      O'rtacha: {formatCurrency(quickActionData.summary.averagePurchasePricePerLiter)} / litr
                    </p>
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">{getSalesSummaryTitle(quickActionData.summary.salesQuantityByUnit)}</CardTitle>
                    <TrendingUp className="h-4 w-4 text-muted-foreground" />
                  </CardHeader>
                  <CardContent>
                    <div className="text-xl font-bold">{formatSalesUnitBreakdown(quickActionData.summary.salesQuantityByUnit)}</div>
                    <p className="text-xs text-muted-foreground">
                      {formatCurrency(quickActionData.summary.totalSalesRevenue)} {getSalesRevenueLabel(quickActionData.summary.salesQuantityByUnit)}
                    </p>
                    {hasLiterSales(quickActionData.summary.salesQuantityByUnit) && (
                      <p className="text-xs text-muted-foreground">
                        Litr o'rtacha: {formatCurrency(quickActionData.summary.averageSalesPricePerLiter)} / litr
                      </p>
                    )}
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {formatMargin(quickActionData.summary.grossProfit, quickActionData.summary.totalSalesRevenue)}% marja
                    </p>
                  </CardContent>
                </Card>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Kunlik sotuv miqdori</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <ResponsiveContainer width="100%" height={300}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Jami sotuv miqdori va daromad</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      {quickActionData.customerBreakdown.map((customer, index) => (
                        <div key={customer.customerId} className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            <div className="text-sm font-medium">{formatSalesUnitBreakdown(customer.salesQuantityByUnit)}</div>
                            <div className="text-xs text-muted-foreground">{formatCurrency(customer.totalRevenue)}</div>
                          </div>
                        </div>
                      ))}
